...
class Surveillance extends Doctrine_Record {
	public function setTableDefinition() {
		...
		$this -> hasColumn('Date_Reported', 'date');
	...
	//dave graph
	public static function graphWeeklyDiseaseSummaries($year, $disease) {
		$query = Doctrine_Query::create() -> select("sum(Lfcase+Lmcase+Gfcase+Gmcase) as Cases, sum(Lfdeath+Lmdeath+Gfdeath+Gmdeath) as Deaths") -> from("surveillance") -> where("Reporting_Year = '$year' and Disease = '$disease'") -> GroupBy("epiweek");
		$surveillance = $query -> execute();
	...
	public function tableEpidemicProneDiseases($epiweek,$disease,$year){
		$query = Doctrine_Query::create() -> select("Lmcase,Gmcase,Lfcase,Gfcase,Surveillance.Disease as Disease,Surveillance.District as District") -> from ("Surveillance,District,Disease") -> where ("Epiweek = '$epiweek' and Disease = '$disease' and Reporting_Year = '$year' and abs(Lmcase) != 0 and abs(Gmcase) != 0 and abs(Lfcase) != 0 and ABS(Gfcase) != 0") -> OrderBy ("ABS(District) ASC");
		$result = $query -> execute();
		//echo $query -> getSQL();
		return $result;
	}
